...
export const DeletePage = () => {
            ...
            if (userData) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        await axios.delete(`http://localhost:3000/api/delete/${userData.idNumber}`);
                        setShowCards(false);
    ...
    return (
        <>
            <Helmet>
                <title>Delete | Data Management</title>
            </Helmet>
            <ScrollArea className="w-full xl:h-max" type="auto">
                <main className="gap-4 mt-8 md:gap-6 xl:container">
                    <div className="grid flex-1 gap-4">
                        <Heading
                            icon={<IoBackspaceSharp size={28} />}
                            title="Delete User"
                            description="Search for a user to delete it from the database."
                        />
