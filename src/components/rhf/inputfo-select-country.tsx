...
import { RenderTwemoji } from '../shared';
...
const Menu = ({ children, ...rest }: any) => {
  return (
    <components.Menu
      className={cn(
        'z-10 mb-2.5 p-1 mt-1 border border-base-content/30 bg-inherit bg-base-100 rounded-md animate-in fade-in duration-500'
      )}
      {...rest}
    >
      {children}
...
const SingleValue = ({ data, ...props }: any) => (
  <components.SingleValue {...props}>
    <div className='pl-0.5 flex flex-row items-center'>
      <RenderTwemoji
        emoji={data.icon}
        className='flex w-[22px] min-w-[22px] h-[22px] overflow-hidden rounded-full'
      />
      <p className='pl-2 truncate'>{data.label}</p>
    </div>
    ...
    <components.Option {...props} className='custom-option'>
      <div className='flex flex-row items-center '>
        <RenderTwemoji
          ...
          className='flex w-[22px] min-w-[22px] h-[22px] overflow-hidden rounded-full'
        />
        <p className='pl-2 truncate text-base font-normal'>{`${data.label} (${data.iso2}) +${data.phoneCode}`}</p>
      </div>
    </components.Option>
  );
};
export interface InputSelectCountryFOProps extends Props {
  id: string;
  name: string;
  options: any;
  control: any;
...
const InputSelectCountryFO = ({
  ...
  ...props
}: InputSelectCountryFOProps) => {
  console.log('Render InputSelectCountryFO');
...
InputSelectCountryFO.displayName = 'InputSelectCountryFO';

export { InputSelectCountryFO };
